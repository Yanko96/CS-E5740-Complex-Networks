    ...
    for datavec, label, style in zip(datavecs,labels, styles):
        ...
        for i, val in enumerate(sorted_datavec):
            for el in sorted_datavec:
                ccdf[i] = (el >= val) + ccdf[i]
            ccdf[i] = ccdf[i] / len(sorted_datavec)
...
def get_link_overlap(net):
    ...
    links = net.edges()
    for i, j in links:
        neighbor_count = len([val for val in net.neighbors(i) if val in net.neighbors(j)])
        k_i = nx.degree(net, i)
        k_j = nx.degree(net, j)
        ...
        if neighbor_count==0:
            ...
            overlap = neighbor_count / (k_i + k_j - neighbor_count - 2)
...
if __name__ == '__main__':
    ...
    xlabel = 'Degree, strength or link weight' ### TIP: label the axis!
    ...
    # YOUR CODE HERE
    av_weight = [strength/degree for strength, degree 
