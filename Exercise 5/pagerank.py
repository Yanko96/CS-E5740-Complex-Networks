...
def pageRank(network, d, n_steps):
    ...
    random_Walker = np.random.choice(a = nodes)
    np.random.seed(765756)
    for i in range(n_steps):
        pageRank[random_Walker]+=1
        prob=np.random.rand()
        if prob>d:
            random_Walker=np.random.choice(nodes)
        else:
            random_Walker=np.random.choice(list(nx.neighbors(network,random_Walker)))
...
def pagerank_poweriter(g, d, iterations):
    ...
    # nodes=list(g.nodes())
    # n_nodes=len(nodes)
    # page_rank_vector=[1/n_nodes for _ in range(n_nodes)]
    # page_rank_vector=np.array(page_rank_vector)
    # random_Jump_vector=page_rank_vector.copy()
    # # compute google matrix
    # google_Matrix=np.zeros((n_nodes,n_nodes))
    # for i in range(n_nodes):
    #     predecessors = [pred for pred in g.predecessors(nodes[i])]
    #     for j in range(n_nodes):
    #         if i==j:
    #             google_Matrix[i,j]=0
    #         elif nodes[j] in predecessors:
    #             google_Matrix[i,j]=1/g.out_degree(nodes[j])
    #         else:
    #             google_Matrix[i,j]=0
    # # begin the interation
    # #print(google_Matrix)
    # epsilon=0
    # new_page_rank_vector=np.zeros(n_nodes)
    # for i in range(iterations):
    #     new_page_rank_vector=d*np.dot(google_Matrix,page_rank_vector)+random_Jump_vector*(1-d)
    #     print('PageRank sums to ')
    #     print(np.sum(new_page_rank_vector))
    #     epsilon=np.sum(np.abs(new_page_rank_vector-page_rank_vector))
    ...
    # result={}
    # for node in nodes:
    #     result[node]=new_page_rank_vector[nodes.index(node)]
    # #print(result)
    # return result  # TODO replace this!
    nodes=list(g.nodes())
    number_nodes=len(nodes)
    page_rank_vector=dict.fromkeys(nodes, 1/number_nodes)

    for i in range(iterations):
        page_rank_old = page_rank_vector.copy()

        for j in nodes:
            in_edges = g.in_edges(j)
            if len(in_edges) != 0:
                neighbours = [edge[0] for edge in in_edges]
                neighbours_contribution = sum([page_rank_vector[j] / g.out_degree(j) for j in neighbours])
            else:
                neighbours_contribution = 0 
            page_rank_vector[j] = (1-d)/number_nodes + d * neighbours_contribution

        # sanity checks in each itteration:
        print(f'PageRank sums to {sum(page_rank_vector.values())}')
        print(f'PageRank difference since last iteration:') 
        print(f'{sum([abs(page_rank_vector[i]-page_rank_old[i]) for i in g])}')
...
def investigate_d(network, ds, colors, n_steps):
    ...
    for i in range((len(ds))):
        result=pageRank(network=network,d=ds[i],n_steps=n_steps)
        results=np.ones(n_nodes)
        for node in result:
            results[int(node)]=result[node]
        plt.plot(results,label=np.round(ds[i], 2),color=colors[i])
...
if __name__ == '__main__':
    ...
    network_path = '/home/yangzhe/桌面/CN2019_ex5/pagerank_network.edg'
    ...
    fig.savefig('/home/yangzhe/桌面/CN2019_ex5/network_visualization.pdf')
    ...
    fig.savefig('/home/yangzhe/桌面/CN2019_ex5/network_visualization_pagerank_rw.pdf')
    ...
    fig.savefig('/home/yangzhe/桌面/CN2019_ex5/network_visualization_pagerank_nx.pdf')
    ...
    fig.savefig('/home/yangzhe/桌面/CN2019_ex5/network_visualization_pagerank_pi.pdf')
    ...
    # YOUR CODE HERE
    def k5net(power):
        return nx.directed_configuration_model(in_degree_sequence=10**power*[5],out_degree_sequence=10**power*[5],create_using=nx.DiGraph())
    ...
    for i in range(num_tests):
        k5_net=nx.directed_configuration_model(in_degree_sequence=10**4*[5],out_degree_sequence=10**4*[5],create_using=nx.DiGraph())
        start=time.time()
        pagerank_poweriter(k5_net,d=0.85,iterations=10)
        end=time.time()
        escape = end - start
        escapes.append(escape)
    print(f'Mean time spent for pageranking a network with {n_nodes} nodes using pagerank_poweriter(): {np.mean(escapes)}s')
    target=26*(10**6)
    print(f'Estimated time for a network with {target} nodes using pagerank_poweriter(): {np.mean(escapes)/n_nodes*target/3600} hours')

    # Investigating the running time of the random walker function
    n_nodes = 10
    # YOUR CODE HERE
    n_steps = 10000 # TODO: set such number of steps that each node gets visited on average 1000 times
    escapes=[]
    for i in range(num_tests):
        k5_net=nx.directed_configuration_model(in_degree_sequence=n_nodes*[5],out_degree_sequence=n_nodes*[5],create_using=nx.DiGraph())
        start=time.time()
        pageRank(k5_net,d=0.85,n_steps=n_steps)
        end=time.time()
        escape = end - start
        escapes.append(escape)
    print(f'Mean time spent for pageranking a network with {n_nodes} nodes using pagerank(): {np.mean(escapes)}s')
    print(f'Estimated time for a network with {target} nodes using pagerank(): {np.mean(escapes)/n_nodes*target/3600} hours')

    # Investigating effects of d:
    ds = np.arange(0, 1.2, 0.2)
    colors = ['b', 'r', 'g', 'm', 'k', 'c']
    # YOUR CODE HERE
    network_path_wp = '/home/yangzhe/桌面/CN2019_ex5/wikipedia_network.edg' # TODO: replace, set the correct network path
    ...
    fig=investigate_d(network=network, ds=ds, colors=colors, n_steps=n_steps)
    fig.savefig('/home/yangzhe/桌面/CN2019_ex5/investigating_d.pdf')
