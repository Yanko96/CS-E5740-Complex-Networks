...
def get_susceptibility(component_size_distribution):
    ...
    # YOUR CODE HERE
    #TODO: Implement this function
    for component_size in component_size_distribution:
        component_number=component_size_distribution[component_size]
        numerator += component_size**2*component_number
        denominator += component_size*component_number
    max_component_size = get_largest_component_size(component_size_distribution)
    numerator -= max_component_size**2
    denominator -= max_component_size
...
def expand_breadth_first_search(network, visited_nodes, boundary_nodes):
    ...
    for node in boundary_nodes:
        for nearest_neghbor in network.neighbors(node):
            if nearest_neghbor not in visited_nodes:
                new_boundary.add(nearest_neghbor)
                visited_nodes.add(nearest_neghbor)
