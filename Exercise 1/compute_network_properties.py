...
def visualize_network(network, figure_title):
    ...
    nx.draw(network,with_labels = True) # networkx command for drawing the network
...
def clustering_and_average_clustering(network):
    ...
    average_clustering = np.mean(clustering_coefficients) # Replace this with a correct definition
...
def density(network):
    ...
    """
    # YOUR CODE HERE
    D = 2*len(list(network.edges()))/(len(list(network.nodes()))*(len(list(network.nodes()))-1))
...
def get_degrees(network):
    ...
    nodes=network.nodes()
    for node in nodes:
        degrees.append(len(list(network.neighbors(node))))
...
def create_degree_clustering_scatter(degrees, clustering, x_label, y_label):
    ...
    ax.scatter(degrees,clustering,alpha=0.5)
    ax.set_xlabel(x_label) # sets the label of the x axis
    ax.set_ylabel(y_label) # sets the label of the y axis
    return fig
...
def load_network(network_fname):
    ...
    net=nx.read_weighted_edgelist(network_fname, comments='#', delimiter=None, create_using=None, nodetype=None, encoding='utf-8')
...
if __name__ == '__main__':
    ...
    degree_distribution_x_label = 'Degree of the nodes' # replace
    degree_distribution_y_label = 'Distribution of Degree' # replace
    ...
    cdf_x_label = 'Degree of the nodes' # replace
    cdf_y_label = 'Complementary Cumulative Degree Distribution' # replace
    ...
    l_nx = nx.algorithms.shortest_paths.generic.average_shortest_path_length(network) # replace
    ...
    degree_clustering_x_label = 'Degree of the nodes' # replace
    degree_clustering_y_label = 'Clustering Coefficient of the nodes' # replace
